...
/*---------------------------------------------------------------------------*/
static void createDynBind(void) {
...
/*---------------------------------------------------------------------------*/
static void onDataChangeEvent(App *app, Event *e) {
    unref(e);
...
static Layout *createMultiLayout(App *app) {
    ...
    layout_dbind(layout1, listener(app, onDataChangeEvent, App), App);
...
/*---------------------------------------------------------------------------*/
static void onDrawFullView(App *app, Event *e) {
    const EvDraw *p = event_params(e, EvDraw);
    char_t text[128];
...
/*---------------------------------------------------------------------------*/
static void onKeyDownFullView(App *app, Event *e) {
...
/*---------------------------------------------------------------------------*/
static Layout *createFullLayout(App *app) {
    Layout *layout = layout_create(1, 1);
    View *view = view_create();
    view_OnDraw(view, listener(app, onDrawFullView, App));
    view_OnKeyDown(view, listener(app, onKeyDownFullView, App));
...
static Panel *createCentralPanel(App *app) {
    ...
    Layout *layout2 = createFullLayout(app);
